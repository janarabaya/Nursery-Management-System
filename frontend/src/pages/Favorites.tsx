...
import './Plants.css';
import { CategoryFilter, PlantCategory } from '../components/CategoryFilter';
...
export function Favorites() {
  ...
  const plants: Plant[] = [
    ...
    },
    {
      ...
      imageUrl: '', // TODO: Add Pothos plant image URL here
      category: ['flower'],
      isPopular: true,
    },
  ];
  ...
  // Filter plants to show only favorites, then by category
  ...
  // Clean up favorites on mount - remove invalid entries
  useEffect(() => {
    const saved = localStorage.getItem('plantFavorites');
    if (saved) {
      const parsed = JSON.parse(saved);
      // Filter out strings and keep only numbers (IDs)
      const validIds = Array.isArray(parsed) ? parsed.filter((item: any) => typeof item === 'number') : [];
      // Also filter out IDs that don't exist in plants array
      const plantIds = plants.map(p => p.id);
      const cleanedFavorites = validIds.filter((id: number) => plantIds.includes(id));
      
      // Update localStorage if cleaned list is different
      if (JSON.stringify(cleanedFavorites) !== JSON.stringify(parsed)) {
        localStorage.setItem('plantFavorites', JSON.stringify(cleanedFavorites));
        setFavorites(cleanedFavorites);
      } else if (JSON.stringify(cleanedFavorites) !== JSON.stringify(favorites)) {
        setFavorites(cleanedFavorites);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Listen for storage events (when favorites are updated from other tabs/components)
    ...
    const interval = setInterval(() => {
      ...
      if (saved) {
        ...
        if (JSON.stringify(cleanedFavorites) !== JSON.stringify(favorites)) {
          setFavorites(cleanedFavorites);
    ...
    <div className="plants-page">
      ...
      <div className="plants-container">
        <div className="plants-header">
          ...
          <p>All the plants you've added to your favorites</p>
        </div>
        ...
        {filteredPlants.length === 0 && (
          <div className="no-plants">
            <p>No favorite plants found. Start adding plants to your favorites!</p>
            <button 
              className="plant-buy-btn" 
