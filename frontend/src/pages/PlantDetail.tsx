...
export function PlantDetail() {
  ...
  // Mock plants data - same as in Plants.tsx
  const plants: Plant[] = [
    {
      id: 1,
      name: 'Olive Seedling',
  ...
  useEffect(() => {
    ...
    console.log('PlantDetail - plantId:', plantId);
    if (plantId) {
      const foundPlant = plants.find(p => p.id === plantId);
      console.log('PlantDetail - foundPlant:', foundPlant);
  ...
  const isAvailable = (plant.quantity || 0) > 0;
  const availabilityText = isAvailable 
    ? `In Stock (${plant.quantity} units available)` 
    : 'Out of Stock';
    ...
    <div className="plant-detail-container">
      ...
      <div className="plant-detail-content">
        ...
        <div className="plant-detail-info">
          ...
          <div className={`plant-availability ${isAvailable ? 'available' : 'out-of-stock'}`}>
            {availabilityText}
          </div>
          ...
          <div className="plant-detail-category">
            <h2>Category</h2>
            <p>{plant.category.join(', ')}</p>
          </div>
          
          {plant.isPopular && (
            <div className="plant-detail-popular">
              <span>⭐ Popular Plant</span>
            </div>
          )}
        </div>
      </div>
